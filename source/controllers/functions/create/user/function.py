...
from source.controllers.processors.queries import query_object_user_by_username
...
def register_page():
    ...
    if form.validate_on_submit():
        ...
        is_empty = query_object_user_by_username(username=get_username)
        if is_empty is None:
            new_user = User(username=get_username, password=get_password)
            database.session.add(new_user)
            database.session.commit()
            return redirect(url_for('server.login.login_page_'))
